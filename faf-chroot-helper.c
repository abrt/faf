/*
 * faf-chroot-helper.c: help faf perform tasks needing root privileges
 *
 * Copied from mach-helper.c.
...
usage ()
{
  printf ("Usage: faf-chroot-helper [command]\n");
  exit (1);
}
...
do_command (const char *filename, char *const argv[])
{
  ...
  char *env[3 + ALLOWED_ENV_SIZE] = {
    [0] = "PATH=/bin:/sbin:/usr/bin:/usr/sbin",
    [1] = "HOME=/root"
  };
  int retval;
  size_t idx=2;
  ...
  */
  ...
  /* add LD_PRELOAD for our selinux lib if FAF_LD_PRELOAD is set, or if
   * /selinux exists */
  /*envvar = getenv ("FAF_LD_PRELOAD");
  if (envvar != 0 || stat ("/selinux", &buf) == 0)
  {
    ld_preload = strdup("LD_PRELOAD=" LIBDIR "/libselinux-faf.so");
    env[idx++] = ld_preload;
    }*/
...
/*
 * allow bind mounts of the selinux dir:
...
do_mount (int argc, char *argv[])
{
  ...
  {
    /* see if it's selinux */
    if (strncmp ("/selinux", argv[4], strlen ("/selinux")) != 0)
