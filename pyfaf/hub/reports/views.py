...
def release_accumulated_history(db, osrelease_ids, component_ids, duration_opt):
    ...
    counts_per_date = (db.session.query(hist_column.label('time'),
                func.sum(hist_table.count).label('count'))
            .group_by(hist_column))

    if osrelease_ids:
        counts_per_date = counts_per_date.filter(
            hist_table.opsysrelease_id.in_(osrelease_ids))

    if component_ids:
        counts_per_date = (counts_per_date.outerjoin(Report,
                Report.id==ReportOpSysRelease.report_id)
    ...
    accumulated_date_counts = (db.session.query(hist_dates.c.time,
                        func.sum(counts_per_date.c.count))
    ...
    hist_mindate = (hist_mindate[0] if not hist_mindate[0] is None
        else datetime.date.today())

    displayed_dates = (d for d in date_iterator(hist_mindate,
        duration_opt, datetime.date.today()))

    if accumulated_date_counts:
        chart_data = (report for report in
            chart_data_generator(accumulated_date_counts, displayed_dates))
    else:
