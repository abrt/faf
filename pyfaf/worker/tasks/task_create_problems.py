...
class CreateProblems(TaskBase):
    ...
    def run(self):
        ...
        child = Popen(["faf-create-problems", "-v"], stdout=PIPE, stderr=STDOUT)
        ...
        child.stdout.close()
        if retcode:
            self.result = "faf-create-problems exitted with {0}".format(retcode)
            raise FailTaskException

        self.result = "Problems created successfully"
