...
class PullTags(TaskBase):
    ...
    def run(self):
        ...
        child = Popen(["faf-koji-pull-tags", "-v", self.args["os"]], stdout=PIPE, stderr=STDOUT)
        ...
        child.stdout.close()
        if retcode:
            self.result = "faf-koji-pull-tags exitted with {0}".format(retcode)
            raise FailTaskException

        self.result = "Tags updated successfully"
