...
class BugtrackerUpdateBugs(Action):
    name = "bugtracker-update-bugs"

    def run(self, cmdline, db):
        if cmdline.tracker:
            tracker = bugtrackers[cmdline.tracker]
            if not tracker.installed(db):
                self.log_error("Bugtracker is not installed")
                return 1

            dbtracker = get_bugtracker_by_name(db, cmdline.tracker)
    ...
    def tweak_cmdline_parser(self, parser):
        parser.add_argument("--tracker", choices=bugtrackers,
                            help="Update bugs only from this tracker")
