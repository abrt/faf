...
class MockYum(Repo):
    """
    An optimized interface to yum repositories. Whenever you are not able use
    regular yum (yum does not work) you can use MockYum repo which behaves like
    yum but it does not need yum because this repository parses repodata files
    on its own.
    """
    ...
    def __init__(self, name, *urls):
        ...
        super(MockYum, self).__init__()

        self.load_config_to_self("mock_yum_cachedir",
                                 ["mock-yum.cachedir"],
                                 "/var/tmp/faf-mock-yum")
        self.name = name
        self.urls = urls

    def _setup_dirs(self, reponame):
        dirname = os.path.join(self.mock_yum_cachedir, self.name, reponame)
        try:
            os.makedirs(dirname)
        except OSError as ex:
            if ex.errno != 17:
                raise FafError("Cache directories error '{0}': {1}"
                               .format(self.mock_yum_cachedir, str(ex)))
        return dirname
    ...
    def _get_repo_file_path(self, reponame, repourl, remote, local=None):
        ...
        cachename = os.path.join(self.mock_yum_cachedir,
                                 self.name,
                                 reponame,
                                 local)
